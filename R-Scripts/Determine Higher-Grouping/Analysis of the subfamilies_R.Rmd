...
give_color <- function(subfamily) {
 if(!is.vector(subfamily)){
  color <- switch(subfamily, "banchi" = "gold1", "ichneu" = "lightskyblue", "ophion" = "blueviolet",  "phygad" = "lightpink","pimpli" = "chartreuse3")}
  
  else{
    color = matrix( NA, length(subfamily))
    for(i in 1:length(subfamily)){
      color[i] <-switch(if(nchar(subfamily[i])>10){substr(subfamily[i],8,13)}else{subfamily[i]}, "banchi" = "gold1", "ichneu" = "lightskyblue", "ophion" = "blueviolet",  "phygad" = "lightpink","pimpli" = "chartreuse3")
...
plot(cva$CVscores, col=give_color(row.names(cva$CVscores)), pch=16,asp=T,
    ...
    las=1, cex=1, main = "CVA of Higher-Groupings")
...
for(jj in 1:length(levels(subfam1))){
  ...
  lines(cva$CVscores[subfam1==ii,1][c(kk, kk[1])],
        cva$CVscores[subfam1==ii,2][c(kk, kk[1])], col=give_color(jj))
...
my_colors1[i] <- give_color(substr(Label[i],1,6))
}
hce %>%
