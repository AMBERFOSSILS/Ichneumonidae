...
proD<-gpagen(landmark, surfaces = NULL, curves= sliders,
             ...
             print.progress = TRUE)
use= c("pimpli", "banchi", "phygad", "ophion")
...
my_colors[i] <- switch(substr(row.names(PCA$pc.scores[,])[i],8,13), "banchi" = "goldenrod1", "ophion" = "blueviolet",  "phygad" = "violet","pimpli" = "chartreuse3")
}

par(mar=c(4, 4, 1, 1)) # sets the margins
plot(PCA$pc.scores[,1],PCA$pc.scores[,2], col=my_colors, asp=T, las=1,pch=16, cex=1.5)
legend("topright", legend= levels(subfam1), col= c("goldenrod1", "blueviolet", "violet", "chartreuse3"), pch=rep(19,14) )
